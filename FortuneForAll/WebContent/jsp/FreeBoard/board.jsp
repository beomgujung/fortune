...
<%
	FreeBoard f = (FreeBoard) request.getAttribute("board");
			...
			<div class="container">
				...
				<%
					if(result==0) {
						%>
				<form method="POST"
					action="/FortuneForAll/com.fortuneforall.freeboard.controller/FreeBoardUpWriteController?id=${id}&num=${board.board_no}&content=${board.content}&title=${board.title}">
					<button>수정</button>
				</form>
				<form method="POST"
					action="/FortuneForAll/com.fortuneforall.freeboard.controller/FreeBoardDeleteController?id=${id}&num=${board.board_no}">
					<button>삭제</button>
				</form>
						<%
					}
